...
class SessionService:
    ...
    async def ensure_session(
        ...
        這是一個冪等 (idempotent) 操作，常在每次接收請求時呼叫。
        """
        resolved_user_id = self._resolve_user_id(user_id)
    ...
    async def update_session(
        ...
        """
        更新特定對話 Session 的狀態。
        實作方式是附加一個系統事件 (System Event) 到該 Session，ADK 處理時會自動合併這些狀態變更。
        這主要用於前端或系統主動推送狀態更新，而不需要透過使用者對話。
        """
        resolved_user_id = self._resolve_user_id(user_id)

        # 確保 Session 存在
        session = await self.ensure_session(session_id, user_id=resolved_user_id)

        # 建立一個包含狀態更新的系統事件
        # 將所有值字串化以符合系統慣例
        stringified_delta: dict[str, Any] = {
            k: safe_stringify(v) for k, v in state_delta.items()
        }

        event = Event(
            id=f"sys-update-{int(time.time() * 1000)}",
            author="system",
            content=genai_types.Content(role="system", parts=[]),
            actions=EventActions(state_delta=stringified_delta),
        )
