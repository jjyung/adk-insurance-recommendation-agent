...
from app.api.routes import run_router, session_router, live_router, auth_router
...
def create_app(container: AppContainer | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        # 初始化觀測性 (Telemetry)
        if config.enable_cloud_tracing or config.enable_cloud_logging:
            hooks = google_cloud.get_gcp_exporters(
                enable_cloud_tracing=config.enable_cloud_tracing,
                enable_cloud_logging=config.enable_cloud_logging,
            )
            # 設定全域 OTel 提供者
        ...
        # 初始化審計日誌
        await app_container.audit_logs.initialize()
    ...
    # 包含應用程式路由器，將會話路由、執行路由與 Live 串流路由整合到應用程式中
