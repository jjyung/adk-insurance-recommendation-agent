...
class AgentRunRequest(BaseModel):
    ...
    image: str | None = Field(
        default=None, description="使用者上傳的圖片 (Base64 編碼)"
    )
    imageType: str | None = Field(
        default=None, description="圖片的 MIME 類型，例如 image/jpeg"
    )
    stream: bool = Field(
        default=True,
        description="是否使用流式 (Streaming) 回傳。若為 False，則文字將累積後一次回傳。",
    )
