...
resource "google_bigquery_dataset" "telemetry_dataset" {
  ...
  dataset_id    = replace("${var.project_name}_telemetry_${var.environment}", "-", "_")
  friendly_name = "${var.project_name} Telemetry (${var.environment})"
  location      = var.region
  description   = "Dataset for GenAI telemetry data stored in GCS"
  depends_on    = [google_project_service.services]
}

# Generate a random suffix for the telemetry bucket name
resource "random_id" "telemetry_bucket_suffix" {
  byte_length = 4
}
...
resource "google_bigquery_connection" "genai_telemetry_connection" {
  project       = var.project_id
  location      = var.region
  connection_id = "${var.project_name}-genai-telemetry-${var.environment}"
  friendly_name = "${var.project_name} GenAI Telemetry Connection (${var.environment})"
...
resource "google_logging_project_bucket_config" "genai_telemetry_bucket" {
  project          = var.project_id
  location         = var.region
  bucket_id        = "${var.project_name}-genai-telemetry-${var.environment}-${random_id.telemetry_bucket_suffix.hex}"
  retention_days   = 3650
...
resource "google_logging_project_sink" "genai_logs_to_bucket" {
  name        = "${var.project_name}-genai-logs-${var.environment}"
  project     = var.project_id
...
resource "google_logging_linked_dataset" "genai_logs_linked_dataset" {
  link_id     = replace("${var.project_name}_genai_telemetry_logs_${var.environment}", "-", "_")
...
# External table for completions data
